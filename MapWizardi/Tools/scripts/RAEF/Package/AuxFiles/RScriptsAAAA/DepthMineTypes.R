...
Row1 <<- cbind(TotalG1, OPG,BCG,RPG,VCRG,NG,Blank,OPGPer,BCGPer,RPGPer,VCRGPer,lNGPer  )
...
TotalG1 <-  (OPG + BCG + NG + RPG)
Total11 <-  (OP1 + BC1 + N1 + RP1)
Total21 <-  (OP2 + BC2 + N2 + RP2)
...
NamesD <- c("Total","Open Pit","Block Caving","Room and Pillar","None","Blank","Open Pit Percent","Block Caving Percent","Room and Pillar Percent", "None Percent")

Row1 <- cbind(TotalG1, OPG,BCG,RPG,NG,Blank,OPGPer,BCGPer,RPGPer,NGPer  )
Row2 <- cbind(Total11, OP1,BC1,RP1,N1,Blank,OP1Per,BC1Per,RP1Per,N1Per  )
Row3 <- cbind(Total21, OP2,BC2,RP2,N2,Blank,OP2Per,BC2Per,RP2Per,N2Per  )
names(Row2) <- names(Row1) 
names(Row3) <- names(Row2) 
DataNew22 <<- rbind(NamesD, Row1, Row2 ,Row3 )

NamesR <<- c("Info","General",paste("Depth Level 1_", Max1,"_Meters",sep="") ,paste("Depth Level 2_", Max2,"_Meters",sep=""))
} ## ends int = 2
...
## End count 1 
TotalG <<-  (OPG + BCG+ NG + RPG)
Total1 <<-  (OP1 + BC1 + N1 + RP1)
Total2 <<-  (OP2 + BC2 + N2 + RP2)
Total3 <<-  (OP3 + BC3 + N3 + RP3)
...
Row1 <<- cbind(TotalG, OPG,BCG,RPG,NG,Blank,OPGPer,BCGPer,RPGPer,NGPer  )
Row2 <<- cbind(Total1, OP1,BC1,RP1,N1,Blank,OP1Per,BC1Per,RP1Per,N1Per  )
Row3 <<- cbind(Total2, OP2,BC2,RP2,N2,Blank,OP2Per,BC2Per,RP2Per,N2Per  )
Row4 <<- cbind(Total3, OP3,BC3,RP3,N3,Blank,OP3Per,BC3Per,RP3Per,N3Per  )


names(Row2) <- names(Row1) 
names(Row3) <- names(Row2) 
names(Row4) <- names(Row2)
DataNew22 <- rbind(NamesD, Row1, Row2 ,Row3 ,Row4)

NamesR <- c("Info","General",paste("Depth Level 1_", Max1,"_Meters",sep="") ,paste("Depth Level 2_", Max2,"_Meters",sep=""),paste("Depth Level 3_", Max3,"_Meters",sep=""))
...
Row3 <<- cbind(Total2, OP2,BC2,RP2,N2,Blank,OP2Per,BC2Per,RP2Per,N2Per  )
Row4 <<- cbind(Total3, OP3,BC3,RP3,N3,Blank,OP3Per,BC3Per,RP3Per,N3Per  )
Row5 <<- cbind(Total4, OP4,BC4,RP4,N4,Blank,OP4Per,BC4Per,RP4Per,N4Per  )
names(Row2) <- names(Row1) 
names(Row3) <- names(Row2) 
names(Row4) <- names(Row2)
...
filenameOut1236 <<- "Depth_MineTypes.csv"
