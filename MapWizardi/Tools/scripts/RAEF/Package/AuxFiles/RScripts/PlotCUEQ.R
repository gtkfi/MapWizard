...
EPSName <<- paste(TN1,"_CUEQ_MetricTons1.eps",sep="")
setEPS()
...
plot(a2, b2, xlab="Ore Tonnage [Log10 t]", ylab="Cut Off Grade [CuEq%]", xlim =c(min(x2,na.rm = TRUE), max(x2,na.rm = TRUE) ), ylim = c(0,yy2), cex= 0,cex.lab=2, cex.axis=1.5, cex.main=2, cex.sub=2,xaxt="n", main=Title1)
aty <- axTicks(2)
atx <- axTicks(1)
...
MyData <- read.csv(file=NewCalcName2 , header=TRUE, sep=",")
## transform data 
...
NumAt <- (NumAt + Divideby)  ### ending lines for each depth level
}
################################################################### Developing Max line
...
JPEGName <<- paste(TN1,"_CUEQ_MetricTons.jpg",sep="")

jpeg(JPEGName, width = 1000, height = 800,quality = 100)
#setEPS()
#postscript("CUEQ_MetricTons1.eps", width= 20, height= 20)
...
TiffName <<- paste(TN1,"_CUEQ_MetricTons.tiff",sep="")
