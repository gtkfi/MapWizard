...
plot.NDepositsPmf <- function( object,
  ...
  if(object$type == "CustomMark3" | object$type == "CustomMark4") {
    ...
    q <- q + ggplot2::theme(legend.position = c(1,1), legend.justification = c(1.1,1.1),axis.text = ggplot2::element_text(size = labelSize),axis.title = ggplot2::element_text(size = labelSize))
    ...
    if(isUsgsStyle) {
      p <- p + ggplot2::ggtitle("A.") +
        ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0,
                                                          face = "italic",size = labelSize))
      q <- q + ggplot2::ggtitle("B.") +
        ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0,
                                                          face = "italic",size = labelSize))
    } else {
      p <- p + ggplot2::ggtitle("(a)") +
        ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0,size = labelSize))
      q <- q + ggplot2::ggtitle("(b)") +
        ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0,size = labelSize))
    }
