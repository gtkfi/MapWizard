...
library(zoo)
library(sqldf)
jpeg("OV_MetricTons2.jpg", width = 1000, height = 800,quality = 100)
FileName <-"NewBEOut10WO0.csv"
FileData2 <- read.csv(file="NewBEOut10WO0.csv", header=TRUE, sep=",")
...
plot(a2, b2, xlab="Ore Tonnage [Log10 t]", ylab="Cut Off Ore Value [2008$/t]", xlim =c(min(x2,na.rm = TRUE), max(x2,na.rm = TRUE) ), ylim = c(min(d2,na.rm = TRUE),max(d2, na.rm = TRUE)), cex= 0,cex.lab=2, cex.axis=1.5, cex.main=2, cex.sub=2,xaxt="n", main=Title1)
aty <- axTicks(2)
atx <- axTicks(1)
